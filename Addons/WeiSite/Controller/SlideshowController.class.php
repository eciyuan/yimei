	...
	public function lists() {
		...
		foreach ( $list_data ['list_data'] as &$vo ) {
	 	...
	 	foreach ($list_data['list_data'] as &$data){
			$map['id']=$data['cate_id'];
			$res=D('weisite_slideshow_category')->where($map)->field('name')->find();
			$data['cate_id']=$res['name'];
		} 
		/* $cateArr=M('weisite_slideshow_category')->getFields('id,name');
		foreach ($list_data['list_data']as&$data){
		    $data ['cate_id'] = $cateArr [$data ['cate_id']];
		} */
