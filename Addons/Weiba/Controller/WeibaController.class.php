...
class WeibaController extends AddonsController {
	...
	function _initialize() {
		...
		$res ['title'] = '版块管理';
		$res ['url'] = addons_url ( 'Weiba://Weiba/lists', array(wecha_id=>get_openid(),token=>get_token()) );
		$res ['class'] = $controller == 'message' ? 'current' : '';
		$nav [] = $res;
		
		$res ['title'] = '论坛配置';
		$res ['url'] = addons_url ( 'Weiba://Weiba/config', array(wecha_id=>get_openid(),token=>get_token()) );
		$res ['class'] = $controller == 'config' ? 'current' : '';
		$nav [] = $res;
		
		$res ['title'] = '预览微论坛';
		$res ['url'] = addons_url ( 'Weiba://Weiba/preview', array(wecha_id=>get_openid(),token=>get_token()));
		$res ['class'] = $controller == 'config' ? 'current' : '';
		$nav [] = $res;
	...
	function preview() {
		$previewUrl = addons_url ( 'Weiba://Wap/index', array (
