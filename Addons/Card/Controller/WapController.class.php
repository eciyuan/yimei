...
// 		$this->assign('shop',$shopInfo);
		//dump($shopInfo);
		//判断是否关注公众号
		$mapSubscribe ['token'] = get_token ();
		$mapSubscribe ['uid'] = $this->mid;
		$has_subscribe = intval ( M ( 'public_follow' )->where ( $mapSubscribe )->getField ( 'has_subscribe' ) );
		$this->assign ( 'has_subscribe', $has_subscribe );
		if ($has_subscribe == 0) { // 获取需要关注的公众号二维码
			$res = D ( 'Home/QrCode' )->add_qr_code ( 'QR_SCENE', 'Card', $info['id'],$mapSubscribe ['uid'] );
