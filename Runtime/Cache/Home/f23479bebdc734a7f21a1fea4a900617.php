            ...
            <ul>
                <?php if(is_array($orderList)): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                        ...
                        <div class="order_list_bottom">
                            <?php if($vo[pay_status]==0): ?><a class="btn small_btn" href="<?php echo U('choose_pay');?>&order_id=<?php echo ($vo["id"]); ?>">立即付款</a>
                                <?php else: ?>
                                <?php if(($vo[is_send]) == "0"): ?><span>等待卖家发货</span>
                                    <?php else: ?>
                                    <?php if($vo['status_code']==3): ?><a class="btn small_btn" href="javascript:;" onClick="confirmGetGoods('<?php echo U('confirm_get',array('id'=>$vo[id]));?>');">确认收货</a>
                                        <?php elseif($vo['status_code']==4): ?>
                                            <a class="btn small_btn" href="<?php echo U('CommentOrder',array('id'=>$vo[id]));?>">立即评价</a>
                                        <?php elseif($vo['status_code']==5): ?>
                                           <a class="btn small_btn"   href="<?php echo U('CommentOrder',array('id'=>$vo[id]));?>">立即评价</a>
                                        <?php elseif($vo['status_code']==6): ?>
                                           <a class="btn small_btn"  href="<?php echo U('CommentOrder',array('id'=>$vo[id]));?>">立即评价</a>
                                        <?php elseif($vo['status_code']==7): ?>
                                          <a class="btn small_btn" >完成交易</a>
                                    <else>
                                     <a class="btn small_btn"  href="<?php echo U('CommentOrder',array('id'=>$vo[id]));?>" >立即评价</a><?php endif; endif; endif; ?>
                            <!--  <a class="btn small_btn " onClick="confirmDelete('<?php echo U('delete_order',array('order_id'=>$vo[id]));?>');" title="删除" >取消订单</a> -->
                        </div>
