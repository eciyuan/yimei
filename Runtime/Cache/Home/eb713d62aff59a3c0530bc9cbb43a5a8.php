...
<div class="container">
    <?php if(empty($lists)): ?><div class="cart_empty">
            ...
            <p>购物车还是空的</p>
            <p><a href="<?php echo U('lists',array('shop_id'=>$shop_id,'token'=>$token));?>">去店里逛逛吧</a></p>
        </div>
        <!-- 底部导航 -->
...
<p class="copyright"><?php echo ($system_copy_right); echo ($tongji_code); ?></p>
    ...
    <?php else: ?>
        ...
        <form action="<?php echo addons_url('Shop://Wap/confirm_order');?>" method="post" onSubmit="return checkCartSubmit()">
            <div class="cart_list">
                <ul>
                    <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ko): $mod = ($i % 2 );++$i;?><p class="title"><?php echo ($ko["business"]); ?></p>
                        <?php if(is_array($ko["child"])): $i = 0; $__LIST__ = $ko["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="cart_item" bid="<?php echo ($ko["bid"]); ?>">
                                <input class="custom_check" id="item_<?php echo ($vo["id"]); ?>" rel="<?php echo ($vo["id"]); ?>" name="goods_ids[]" type="checkbox"  value="<?php echo ($vo["goods"]["id"]); ?>" checked="checked" />
                                <label for="item_<?php echo ($vo["id"]); ?>"><em>&nbsp;</em></label>
                                <div class="goods_item">
                                    <a href="<?php echo U('detail',array('shop_id'=>$shop_id,'id'=>$vo['goods']['id'],'token'=>$token));?>">
                                        <img class="goods_img" src="<?php echo (get_cover_url($vo["goods"]["cover"])); ?>"/>
                                    </a>
                                    <div class="goods_desc">
                                        <a class="name" href="<?php echo U('detail',array('shop_id'=>$shop_id,'id'=>$vo['goods']['id'],'token'=>$token));?>"><?php echo ($vo["goods"]["title"]); ?></a>
                                        <p class="info"><span class="colorless">重量:</span><span class="singleWeight"><?php echo ($vo["goods"]["weight"]); ?></span> Kg</p>
                                        <p class="info"><span class="colorless">单价:</span><span class="orange">￥<span class="singlePrice"><?php echo (wp_money_format($vo["goods"]["price"])); ?></span></span></p>
                                        <div class="buy_count">
                                            <a class="reduce" href="javascript:;">-</a>
                                            <input type="text" name="buyCount[<?php echo ($vo["goods"]["id"]); ?>]" value="<?php echo (intval($vo["num"])); ?>" rel="buyCount"/>
                                            <a class="add" href="javascript:;">+</a>
                                        </div>
                                        <input type="hidden" name="is_oversea" id="is_oversea" value="<?php echo ($vo['goods']['is_oversea']); ?>"/>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    function deleteCartItem(){
        if($('input[name="goods_ids[]"]:checked').size()==0){
            ...
            $.Dialog.loading();
            $.ajax({
                url:"<?php echo U('delCart',array('shop_id'=>$shop_id,'token'=>$token));?>",
                data:{ids:cartIds},
                dataType:"json",
                type:"post",
