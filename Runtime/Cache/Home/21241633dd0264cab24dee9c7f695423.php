	...
	<form action="<?php echo U('config');?>" class="form-horizontal" method="post">
		<?php if(empty($custom_config)): if(is_array($data['config'])): foreach($data['config'] as $o_key=>$form): ?><div class="form-item cf">
                  ...
                  <?php echo hook('material', array('name'=>$o_key,'value'=>$form['value'],'extra'=>$field['extra'])); break;?>
							...
							<?php case "editor": ?><label class="item-label">正文<span class="check-tips">图片命名不能有特殊符号</span></label>
								<div class="controls">
									<label class="textarea">
										<textarea id="termOld" style="width:405px; height:200px;" name="term" ><?php echo ($form["value"]); ?></textarea>
										<?php echo hook('adminArticleEdit', array('name'=>term,'value'=>$form['value']));?> </label>
									</label>
								</div><?php break;?>
